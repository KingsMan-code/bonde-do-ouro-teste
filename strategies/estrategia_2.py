"""
Estratégia 2: Confirmação de Volume + Médias Móveis
- Entrada: Golden Cross + Volume acima da média (confirmação de volume)
...
def simular_estrategia_2(symbol: str, percentual_entrada: float, interval: str, df_candles: pd.DataFrame) -> dict:
    """
    Simula a estratégia com confirmação de volume
...
def _simulate_single_combo(df: pd.DataFrame, ma_short: int, ma_long: int, 
    ...
    """
    Simula uma única combinação de MAs com confirmação de volume
    """
    if df.empty:
        return {
    ...
    df_with_indicators['estrategia'] = "confirmacao_volume"
    ...
    for i in range(1, len(df_with_indicators)):
        ...
        # Verifica se há dados suficientes para as MAs e volume
        if (pd.isna(current_row['ma_short']) or pd.isna(current_row['ma_long']) or 
            pd.isna(current_row['volume_ma_20'])):
            continue
        
        # Sinal de entrada: Golden Cross + Confirmação de Volume
        if not position_open:
            ...
            # Confirmação de volume: volume atual 50% acima da média
            volume_confirmation = current_row['volume'] > (current_row['volume_ma_20'] * 1.5)
            
            if golden_cross and volume_confirmation:
                position_open = True
                entry_price = current_price
                
                # Marca no DataFrame
                df_with_indicators.iloc[i, df_with_indicators.columns.get_loc('signal_buy_price')] = entry_price
                df_with_indicators.iloc[i, df_with_indicators.columns.get_loc('reason')] = "golden_cross_volume_confirm"
